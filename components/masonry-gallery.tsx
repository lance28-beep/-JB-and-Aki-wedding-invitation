  ...
  return (
    <div ref={topRef} className="relative">
      ...
      <div className="mb-6 flex justify-end">
        <div className="text-[#800A06]/80 text-sm font-[family-name:var(--font-crimson)] tracking-wide">
      ...
      {/* Masonry grid */}
      {filtered.length === 0 ? (
        <div className="text-center text-[#800A06]/70 font-[family-name:var(--font-crimson)]">No images to display.</div>
      ) : (
        <div className="columns-2 sm:columns-3 md:columns-4 lg:columns-5 gap-3 sm:gap-4">
          ...
          >
            <div className="relative w-full overflow-hidden rounded-xl border border-[#9F8650]/30 bg-white/60 backdrop-blur-sm shadow-lg hover:shadow-xl transition-all duration-300 hover:border-[#9F8650]/50">
              {!loaded[img.src] && (
                <div className="aspect-[3/4] sm:aspect-[4/5] w-full animate-pulse bg-gradient-to-br from-white/40 via-[#EFCA93]/30 to-white/20" />
              )}
              <img
      ...
      {lightboxIdx != null && filtered[lightboxIdx] && (
        ...
        >
          <div className="relative max-w-6xl w-full flex items-center justify-center" onClick={(e) => e.stopPropagation()}>
            <button
              className="absolute left-2 sm:left-4 top-1/2 -translate-y-1/2 text-white bg-black/50 hover:bg-black/70 border border-white/20 hover:border-white/40 rounded-full px-4 py-2.5 transition-all duration-200 shadow-lg hover:scale-110"
            ...
            />
            <button
              className="absolute right-2 sm:right-4 top-1/2 -translate-y-1/2 text-white bg-black/50 hover:bg-black/70 border border-white/20 hover:border-white/40 rounded-full px-4 py-2.5 transition-all duration-200 shadow-lg hover:scale-110"
              onClick={() => setLightboxIdx((i) => (i == null ? null : (i + 1) % filtered.length))}
            >
              ›
            </button>
            <button
              className="absolute top-3 right-3 text-white bg-black/50 hover:bg-black/70 border border-white/20 hover:border-white/40 rounded-full px-4 py-2 transition-all duration-200 shadow-lg hover:scale-105 text-sm"
              onClick={() => setLightboxIdx(null)}
            >
              Close
            </button>
      ...
      <div className="mt-8 flex justify-center">
        <button
          type="button"
          className="px-6 py-3 rounded-full bg-gradient-to-r from-[#9F8650] to-[#9F8650]/90 text-white font-semibold border border-[#9F8650] hover:from-[#9F8650]/90 hover:to-[#9F8650] hover:scale-105 transition-all duration-200 shadow-lg hover:shadow-xl"
